        ...
        if serializer.is_valid():
            try:
                ...
                order = Order.objects.get(
                    id=order_id, user=request.user, status="pending"
                )

                # Create payment
                payment = PaymentService.create_payment(
                    user=request.user, order=order, payment_provider=payment_provider
                )
                ...
                return Response(
                    {"payment": PaymentSerializer(payment).data, **result},
...
def paystack_webhook(request):
    ...
    webhook, created = PaymentWebhook.objects.get_or_create(
        ...
        defaults={"event_type": event.get("event"), "data": event.get("data", {})},
    )
